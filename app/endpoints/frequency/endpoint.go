...
func RegisterRoutes(api api.API, enp *Endpoint) {
	api.RegisterRoutes(func(router *mux.Router) {
		// Map the JSON responder to /most-frequent GET requests
		router.HandleFunc("/most-frequent", enp.jsonResponder.ServeHTTP).Methods("GET")
	})
}
	...
	return &Endpoint{
		api.JSONResponder{
			...
			ModelFromRequest: func(r *http.Request) (interface{}, error) {
				...
				return CalculateMostFrequentUsers(reader, dateRange)
			},
		},
		reader,
	}
